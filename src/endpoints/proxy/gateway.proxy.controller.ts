...
export class GatewayProxyController {
  ...
  @Get('/address/:address/dcdt')
  async getAddressDcdt(@Param('address', ParseAddressPipe) address: string) {
    // eslint-disable-next-line require-await
    return await this.gatewayGet(`address/${address}/dcdt`, GatewayComponentRequest.addressDetails, undefined, async (error) => {
