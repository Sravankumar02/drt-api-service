...
export async function issueDcdt(args: IssueDcdtArgs) {
  ...
  const dcdtIssueLog = txResponse?.data?.data?.transaction?.logs?.events?.find(
    (event: { identifier: string }) => event.identifier === 'issue',
  );
  const tokenIdentifier = Buffer.from(
    dcdtIssueLog.topics[0],
    'base64',
  ).toString();
  console.log(
...
export async function issueMultipleDcdts(
  chainSimulatorUrl: string,
  issuer: string,
  numTokens: number,
) {
  ...
  for (let i = 1; i <= numTokens; i++) {
    ...
    const tokenIdentifier = await issueDcdt(
      new IssueDcdtArgs({
        chainSimulatorUrl,
        issuer,
        tokenName,
  ...
  for (const { identifier: tokenIdentifier, type } of nftCollectionIdentifiers) {
    ...
    if (type === 'sft') {
      roles.push(
        ...
        Buffer.from('DCDTTransferRole').toString('hex')
      );
    } else {
      roles.push(
        Buffer.from('DCDTRoleNFTCreate').toString('hex'),
        Buffer.from('DCDTRoleNFTBurn').toString('hex'),
        Buffer.from('DCDTRoleNFTUpdateAttributes').toString('hex'),
        Buffer.from('DCDTRoleNFTAddURI').toString('hex'),
        Buffer.from('DCDTTransferRole').toString('hex')
      );
    }
...
export async function issueMultipleMetaDCDTCollections(
  chainSimulatorUrl: string,
  issuer: string,
  numberOfCollections: number,
  tokensPerCollection: number
): Promise<string[]> {
  const metaDcdtCollectionIdentifiers: { identifier: string }[] = [];
  ...
  for (let i = 0; i < numberOfCollections; i++) {
    ...
    const dcdtIssueLog = txResponse?.data?.data?.transaction?.logs?.events?.find(
      (event: { identifier: string }) => event.identifier === 'registerMetaDCDT',
    );
    ...
    if (dcdtIssueLog) {
      ...
      console.log(
        `Issued MetaDCDT collection ${tokenName}. tx hash: ${txHash}. identifier: ${tokenIdentifier}`,
      );

      // Set special roles for the MetaDCDT collection
      ...
      // Create MetaDCDT tokens for this collection
      for (let j = 1; j <= tokensPerCollection; j++) {
        ...
        console.log(
          `Created MetaDCDT${j} for collection ${tokenIdentifier}. tx hash: ${createTxHash}`
        );
      }
    }
  }
