...
describe('TransactionGetService', () => {
  ...
  const mockSender = 'drt1qga7ze0l03chfgru0a32wxqf2226nzrxnyhzer9lmudqhjgy7ycq0wn4su';
  const mockReceiver = 'drt15hmuycqw4mkaksfp0yu0auy548urd0wp6wyd4vtjkg3t6h9he5ysk8an0y';
  ...
  describe('getTransactionLogsFromElasticInternalEventsIndex', () => {
    ...
    it('should transform events data correctly', async () => {
      ...
      if (hash1Log) {
        ...
        expect(hash1Log.address).toBe('drt1addr1');
        expect(hash1Log.events).toHaveLength(2);

        const firstEvent = hash1Log.events[0];
        expect(firstEvent.identifier).toBe('DCDTTransfer');
        expect(firstEvent.address).toBe('drt1addr1');
  ...
  describe('removeDuplicatedDCDTTransferEvents', () => {
    it('should remove duplicate DCDTTransfer events with identical content', () => {
      const events = [
        new TransactionLogEvent({
          identifier: 'DCDTTransfer',
          ...
          additionalData: ['', 'RENEVFRyYW5zZmVy', 'T05FLTgzYTdjMA==', 'CteOvFrGIAAA'],
        }),
        new TransactionLogEvent({
          identifier: 'writeLog',
          address: 'drt18wegffw8e65mtucy63mmz5pkm80vknjwkrrge277pvpmjf0t98vqjjlqkm',
      ...
      service['removeDuplicatedDCDTTransferEvents'](events);

      expect(events.length).toBe(3);

      const dcdtTransferEvents = events.filter(e => e.identifier === 'DCDTTransfer');
      expect(dcdtTransferEvents.length).toBe(1);
    ...
    it('should not remove DCDTTransfer events with different content', () => {
      const events = [
        new TransactionLogEvent({
          identifier: 'DCDTTransfer',
          address: 'drt1address1',
          topics: ['topic1', 'topic2'],
          data: 'data1',
      ...
      expect(events.filter(e => e.identifier === 'DCDTTransfer').length).toBe(2);
    });

    it('should do nothing when there are no DCDTTransfer events', () => {
      const events = [
        new TransactionLogEvent({
          identifier: 'writeLog',
          address: 'drt1address1',
    ...
    it('should do nothing when there is only one DCDTTransfer event', () => {
      const events = [
        new TransactionLogEvent({
          identifier: 'DCDTTransfer',
          address: 'drt1address1',
      ...
      expect(events[0].identifier).toBe('DCDTTransfer');
    });
  });
  ...
  describe('tryGetTransactionFromGateway', () => {
    const mockGatewayResponse = createMockGatewayTransaction({
      ...
      logs: { events: [] },
      guardian: 'drt1guardian',
      guardianSignature: 'guardianSig',
      relayerAddress: 'drt1relayer',
