...
import { ElasticQuery, QueryOperator, QueryType, QueryConditionOptions, ElasticSortOrder, ElasticSortProperty, RangeGreaterThanOrEqual, MatchQuery } from "@sravankumar02/sdk-nestjs-elastic";
...
export class ElasticIndexerService implements IndexerInterface {
  ...
  async getNfts(pagination: QueryPagination, filter: NftFilter, identifier?: string): Promise<any[]> {
    ...
    if (identifier !== undefined) {
      const [tokensResult, accountsResult] = await Promise.all([
        ...
        this.elasticService.getList('accountsdcdt', 'identifier',
          ElasticQuery.create()
  ...
  async getAllTokensMetadata(action: (items: any[]) => Promise<void>): Promise<void> {
    const query = ElasticQuery.create()
      ...
      .withMustMultiShouldCondition([DcdtType.NonFungibleDCDT, DcdtType.SemiFungibleDCDT, DcdtType.MetaDCDT], type => QueryType.Match('type', type))
      .withPagination({ from: 0, size: 10000 });
