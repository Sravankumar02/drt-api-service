import { AddressUtils, BinaryUtils } from "@sravankumar02/sdk-nestjs-common";
import { AbstractQuery, ElasticQuery, MatchQuery, QueryConditionOptions, QueryOperator, QueryType, RangeGreaterThan, RangeGreaterThanOrEqual, RangeLowerThan, RangeLowerThanOrEqual } from "@sravankumar02/sdk-nestjs-elastic";
...
export class ElasticIndexerHelper {
    ...
    if (address) {
      elasticQuery = elasticQuery.withMustCondition(QueryType.Should(
        [
          ...
          QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTCreate', address)]),
          QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTBurn', address)]),
          QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTAddQuantity', address)]),
          QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTUpdateAttributes', address)]),
          QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTAddURI', address)]),
          QueryType.Nested('roles', [new MatchQuery('roles.DCDTTransferRole', address)]),
        ],
      ));
    }
  ...
  public buildElasticNftFilter(filter: NftFilter, identifier?: string, address?: string): ElasticQuery {
    ...
    if (filter.excludeMetaDCDT === true) {
      elasticQuery = elasticQuery.withMustMultiShouldCondition([...this.nonFungibleDcdtTypes, ...this.semiFungibleDcdtTypes], type => QueryType.Match('type', type));
    }
  ...
  public buildTokensWithRolesForAddressQuery(address: string, filter: TokenWithRolesFilter, pagination?: QueryPagination): ElasticQuery {
    const rolesConditions = [
      QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleLocalMint', address)]),
      QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleLocalBurn', address)]),
      QueryType.Nested('roles', [new MatchQuery('roles.DCDTTransferRole', address)]),
    ];

    if (filter.includeMetaDCDT === true) {
      rolesConditions.push(QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTAddQuantity', address)]));
      rolesConditions.push(QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTAddURI', address)]));
      rolesConditions.push(QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTCreate', address)]));
      rolesConditions.push(QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTBurn', address)]));
      rolesConditions.push(QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleNFTUpdateAttributes', address)]));
    }
    ...
    if (filter.includeMetaDCDT === true) {
      elasticQuery = elasticQuery.withMustMultiShouldCondition([DcdtType.FungibleDCDT, DcdtType.MetaDCDT], type => QueryType.Match('type', type));
    } else {
      elasticQuery = elasticQuery.withMustMatchCondition('type', DcdtType.FungibleDCDT);
    }
    ...
    if (filter.canMint !== undefined) {
      ...
      elasticQuery = elasticQuery.withCondition(condition, QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleLocalMint', address)]));
    }

    if (filter.canBurn !== undefined) {
      const condition = filter.canBurn === true ? QueryConditionOptions.must : QueryConditionOptions.mustNot;
      elasticQuery = elasticQuery.withCondition(condition, QueryType.Nested('roles', [new MatchQuery('roles.DCDTRoleLocalBurn', address)]));
    }
