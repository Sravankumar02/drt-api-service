...
import { BinaryUtils, ContextTracker } from "@sravankumar02/sdk-nestjs-common";
import { ApiService, ApiSettings } from "@sravankumar02/sdk-nestjs-http";
...
export class GatewayService {
  private readonly snapshotlessRequestsSet: Set<String> = new Set([
    ...
    GatewayComponentRequest.addressDcdt,
  ...
  private readonly deepHistoryRequestsSet: Set<String> = new Set([
    GatewayComponentRequest.addressDetails,
    GatewayComponentRequest.addressDcdt,
    GatewayComponentRequest.addressDcdtBalance,
    GatewayComponentRequest.addressNftByNonce,
    GatewayComponentRequest.vmQuery,
  ]);
  ...
  async getDcdtSupply(identifier: string): Promise<DcdtSupply> {
    const result = await this.get(`network/dcdt/supply/${identifier}`, GatewayComponentRequest.dcdtSupply);
    return result;
  }

  async getDcdtFungibleTokens(): Promise<string[]> {
    const result = await this.get('network/dcdt/fungible-tokens', GatewayComponentRequest.allFungibleTokens);
    return result.tokens;
  }

  async getAddressDcdtRoles(address: string): Promise<DcdtAddressRoles> {
    const result = await this.get(`address/${address}/dcdts/roles`, GatewayComponentRequest.addressDcdtAllRoles);
    return result;
  }
  ...
  async getAddressDcdt(address: string, identifier: string): Promise<TokenData> {
    // eslint-disable-next-line require-await
    const result = await this.get(`address/${address}/dcdt/${identifier}`, GatewayComponentRequest.addressDcdtBalance, async (error) => {
      const errorMessage = error?.response?.data?.error;
  ...
  async getAddressNft(address: string, identifier: string): Promise<NftData> {
    ...
    // eslint-disable-next-line require-await
    const result = await this.get(`address/${address}/nft/${dcdtIdentifier}/nonce/${nonceNumeric}`, GatewayComponentRequest.addressNftByNonce, async (error) => {
      const errorMessage = error?.response?.data?.error;
      if (errorMessage && errorMessage.includes('account was not found')) {
        return true;
      }
