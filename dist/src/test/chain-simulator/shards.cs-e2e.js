...
describe('Shards e2e tests with chain simulator', () => {
    describe('GET /shards', () => {
        it('should return status code 200 and a list of shards', async () => {
            const response = await axios_1.default.get(`${env_config_1.config.apiServiceUrl}/shards`);
    ...
    describe('GET /shards with pagination', () => {
        ...
        it('should return paginated shards with custom size', async () => {
            const size = 2;
            const response = await axios_1.default.get(`${env_config_1.config.apiServiceUrl}/shards?size=${size}`);
            expect(response.status).toBe(200);
            expect(response.data).toBeInstanceOf(Array);
        ...
        it('should return paginated shards with custom from', async () => {
            const from = 1;
            const response = await axios_1.default.get(`${env_config_1.config.apiServiceUrl}/shards?from=${from}`);
            expect(response.status).toBe(200);
            expect(response.data).toBeInstanceOf(Array);
        ...
        it('should return paginated shards with both from and size', async () => {
            const from = 1;
            const size = 2;
            const response = await axios_1.default.get(`${env_config_1.config.apiServiceUrl}/shards?from=${from}&size=${size}`);
            expect(response.status).toBe(200);
            expect(response.data).toBeInstanceOf(Array);
            expect(response.data.length).toBe(size);
        });
        ...
        it('should contain metachain shard (4294967295)', async () => {
            const response = await axios_1.default.get(`${env_config_1.config.apiServiceUrl}/shards`);
            const shards = response.data;
            const metachainShard = shards.find((shard) => shard.shard === 4294967295);
            expect(metachainShard).toBeDefined();
            expect(metachainShard === null || metachainShard === void 0 ? void 0 : metachainShard.validators).toBeGreaterThanOrEqual(1);
            expect(metachainShard === null || metachainShard === void 0 ? void 0 : metachainShard.activeValidators).toBeGreaterThanOrEqual(1);
        });
    });
});
//# sourceMappingURL=shards.cs-e2e.js.map